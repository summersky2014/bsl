import { stringHandlers as defaultStringHandlers } from './inject';
import OrderedElements from './ordered-elements';
import staticData from './staticPrefixData';
import { SelectorHandler, SheetDefinition, StringHandlers } from './type';
import { kebabifyStyleName, stringifyValue } from './util';
// eslint-disable-next-line @typescript-eslint/ban-types
type GenerateSubtreeStyles = (value: string) => string[];
const createPrefixer = require('inline-style-prefixer/lib/createPrefixer').default;
...
export const defaultSelectorHandlers: SelectorHandler[] = [
  ...
  function pseudoSelectors(selector: string, baseSelector: string, generateSubtreeStyles: GenerateSubtreeStyles) {
    if (selector[0] !== ":") {
      return null;
    }
  ...
  function mediaQueries(selector: string, baseSelector: string, generateSubtreeStyles: GenerateSubtreeStyles) {
    if (selector[0] !== "@") {
      return null;
    }
...
export const generateCSS = (
  ...
  stringHandlers = defaultStringHandlers,
): string[] => {
...
const arrayToObjectKeysReducer = (acc: Record<string, unknown>, val: string) => {
  acc[val] = true;
  return acc;
};
...
export const generateCSSRuleset = (
  ...
  const transformValue = stringifyValue;
  const rules = [];
  for (let i = 0; i < declarations.keyOrder.length; i ++) {
    ...
    if (Array.isArray(value)) {
      ...
      for (let j = 0; j < value.length; j++) {
        rules.push(transformRule(key, value[j], transformValue as any));
      }
    } else {
      rules.push(transformRule(key, value, transformValue as any));
    }
  }
