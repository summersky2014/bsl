...
import type { SelectorHandler, SheetDefinition, MaybeSheetDefinition } from './type';
...
const injectStyleTag = (cssRules: string[]) => {
  ...
  // $FlowFixMe
  const sheet = (((styleTag as any).styleSheet || styleTag.sheet /* : any */) /* : CSSStyleSheet */);
...
export const stringHandlers: Record<string, any> = {
  // With fontFamily we look for objects that are passed in and interpret
  // them as @font-face rules that we need to inject. The value of fontFamily
  // can either be a string (as normal), an object (a single font face), or
  // an array of objects and strings.
  fontFamily: function fontFamily(val: Record<string, string> | Record<string, string>[]) {
    if (Array.isArray(val)) {
      ...
      injectStyleOnce(val.src, "@font-face", [val]);
  ...
  animationName: function animationName(val: OrderedElements, selectorHandlers: SelectorHandler[]): any {
    if (Array.isArray(val)) {
      ...
      if (val instanceof OrderedElements) {
        val.forEach((valVal, valKey) => {
          finalVal += generateCSS(
            valKey, [valVal as any as SheetDefinition], selectorHandlers, stringHandlers as any).join('');
        });
      } else {
        Object.keys(val).forEach(key => {
          finalVal += generateCSS(
            key, [val[key]], selectorHandlers, stringHandlers as any).join('');
        });
      }
      finalVal += '}';
...
// set of class names)
let alreadyInjected: Record<string, boolean> = {};
...
const injectGeneratedCSSOnce = (key: string, generatedCSS: string[]) => {
...
const isValidStyleDefinition = (def: Record<string, any>) =>
...
const processStyleDefinitions = (
  ...
  definitionBits: SheetDefinition[],
  length: number
): number => {
...
//   }
  if (styleDefinitions) {
    ...
    } else if (isValidStyleDefinition(styleDefinitions)) {
      classNameBits.push(styleDefinitions._name as string);
      definitionBits.push(styleDefinitions._definition! as any as SheetDefinition);
      length += styleDefinitions!._len as any as number;
    } else {
      throw new Error("Invalid Style Definition: Styles should be defined using the StyleSheet.create method.");
    }
  }
...
export const injectAndGetClassName = (
  styleDefinitions: MaybeSheetDefinition /* : MaybeSheetDefinition[] */,
  selectorHandlers: SelectorHandler[] /* : SelectorHandler[] */
) /* : string */ => {
  const classNameBits: string[] = [];
  const definitionBits: SheetDefinition[] = [];
